# Step 3: Newton's method conjugate gradient
...
@views function jvp!(δϕ, r̄, r, ∇ϕ, ∇ϕ̄, d, d̄, q, q̄, ϕ, ϕ̄, ∇ϕr, m, p, dx)
    make_zero!(∇ϕ̄)
    make_zero!(d̄)
    make_zero!(q̄)
    make_zero!(ϕ̄)
...
@views function increment!(δϕ, r, r̄, k, z, s, ∇ϕ, ∇ϕ̄, d, d̄, q, q̄, ϕ, ϕ̄, ∇ϕr, m, p, maxiter, ncheck, cgtol, nx, dx)
    @. δϕ = 0
    # compute linearised residual
    jvp!(δϕ, r̄, r, ∇ϕ, ∇ϕ̄, d, d̄, q, q̄, ϕ, ϕ̄, ∇ϕr, m, p, dx)
    @. k = r̄ + r
    # compute preconditioned linearised residual
    preconditioner!(z, k, d, dx)
...
@views function main()
    ...
    for inewt in 1:maxnewt
        ...
        increment!(δϕ, r, r̄, k, z, s, ∇ϕ, ∇ϕ̄, d, d̄, q, q̄, ϕ, ϕ̄, ∇ϕr, m, p, maxiter, ncheck, cgtol, nx, dx)
        # update solution
        γ = 0.9
        @. ϕ[2:end-1] += γ * δϕ
    end
